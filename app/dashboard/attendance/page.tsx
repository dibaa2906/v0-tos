...
import { getTodayAttendance, clockIn, clockOut, checkLocationAccess } from "@/lib/attendance"
...
export default function AttendancePage() {
  ...
  const handleVerifyAndClockInInternal = async (image: string) => {
    ...
    try {
      ...
      const record = clockIn(user.id, image, location)
  ...
  const handleVerifyAndClockOutInternal = async (image: string) => {
    ...
    try {
      ...
      const record = clockOut(user.id, image, location)
  ...
  const handleVerifyAndClockIn = async () => {
    ...
    try {
      ...
      const record = clockIn(user.id, capturedImage, location)
      console.log('✅ Attendance record created:', record.id)
  ...
  const handleVerifyAndClockOut = async () => {
    ...
    try {
      ...
      const record = clockOut(user.id, capturedImage, location)
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Record your attendance with photo verification</p>
      </div>
              ...
              {attendance.clockInTime && !attendance.clockOutTime && (
                <div className="p-3 rounded-lg bg-blue-50 border border-blue-200">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-semibold text-blue-900">You have already clocked in today</p>
                      <p className="text-sm text-blue-700">Clock in time: {attendance.clockInTime}</p>
                    </div>
                  </div>
                </div>
              )}
              {attendance.clockInTime && attendance.clockOutTime && (
                <div className="p-3 rounded-lg bg-green-50 border border-green-200">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-semibold text-green-900">Attendance complete for today</p>
                      <p className="text-sm text-green-700">Clock in: {attendance.clockInTime} | Clock out: {attendance.clockOutTime}</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            {location ? (
              locationError ? (
                <>
                  ...
                  <div>
                    <p className="text-sm font-medium text-red-500">Location Error</p>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="relative aspect-[3/4] bg-black rounded-lg overflow-hidden max-w-md mx-auto">
              {cameraActive ? (
                <>
                  <video
                    ref={videoRef}
            ...
            <div className="flex flex-col gap-2">
              {scanning && (
                <div className="p-3 rounded-lg bg-blue-50 border border-blue-200 text-center">
                  <p className="text-sm font-medium text-blue-900">{scanStatus || "🔍 Scanning for face..."}</p>
                  <p className="text-xs text-blue-700 mt-1">Make sure your face is clearly visible</p>
                </div>
              )}
              {isClockOutAllowed && !cameraActive && !capturing && (
                <div className="text-center p-2 rounded-md bg-emerald-50 text-emerald-700 text-sm">
                ...
                <>
                  {isClockInAllowed && (
                    ...
                    >
                      ...
                      {!location ? "Waiting for location..." : "Clock In"}
                    </Button>
                  )}
                  {isClockOutAllowed && (
                    <Button
                    ...
                    >
                      <Camera className="h-4 w-4 mr-2" />
                      {!location ? "Waiting for location..." : "Clock Out"}
                    </Button>
                  )}
                </>
              ) : (
